...
TEST(SegmentManagerTest, SeqWrite) {
  ...
  constexpr int32_t seg_cap = 16;
  constexpr int32_t max_lba = (seg_num - 2) * seg_cap;

  std::shared_ptr<SegmentManager> manager = std::make_shared<SegmentManager>(seg_num, seg_cap);
